...
const DocumentViewer: React.FC = () => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      const [documentsResponse, folderResponse] = await Promise.all([
        fetch(`${import.meta.env.VITE_API_URL}/api/auth/documents/${user.id}`),
        fetch(`${import.meta.env.VITE_API_URL}/api/auth/folder/${user.id}`)
      ]);
  ...
  const downloadDocument = async (documentId: string, originalName: string) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/auth/documents/${user.id}/${documentId}`);
