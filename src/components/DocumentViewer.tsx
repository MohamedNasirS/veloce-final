...
const DocumentViewer: React.FC = () => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      const [documentsResponse, folderResponse] = await Promise.all([
        fetch(`http://147.93.27.172:3001/api/auth/documents/${user.id}`),
        fetch(`http://147.93.27.172:3001/api/auth/folder/${user.id}`)
      ]);
  ...
  const downloadDocument = async (documentId: string, originalName: string) => {
    try {
      const response = await fetch(`http://147.93.27.172:3001/api/auth/documents/${user.id}/${documentId}`);
