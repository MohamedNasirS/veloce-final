...
const BidDetail = () => {
  ...
  const fetchBidDetail = async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://147.93.27.172/api/bids/${bidId}`);
  ...
  const placeBid = async () => {
    ...
    try {
      setPlacingBid(true);
      const response = await fetch(`http://147.93.27.172/api/bids/${bidId}/place-bid`, {
        method: 'POST',
        headers: {
  ...
  const closeBid = async () => {
    ...
    try {
      const response = await fetch(`http://147.93.27.172/api/bids/${bidId}/close`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {bid.images.map((image) => (
                    <div key={image.id} className="relative">
                      <img
                        src={`http://147.93.27.172/api/bids/images/${image.id}`}
                        alt={image.originalName}
                        className="w-full h-32 object-cover rounded-lg border cursor-pointer hover:opacity-80"
                        onClick={() => setSelectedImage(`http://147.93.27.172/api/bids/images/${image.id}`)}
                      />
                    </div>
                  ))}
                </div>
