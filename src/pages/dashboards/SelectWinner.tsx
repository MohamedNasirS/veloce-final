...
export default function SelectWinner() {
    ...
    async function fetchData() {
      ...
      try {
        const [bidResponse, historyResponse] = await Promise.all([
          axios.get(`http://147.93.27.172:3001/api/bids/${bidId}`),
          axios.get(`http://147.93.27.172:3001/api/bids/${bidId}/history`),
        ]);
  ...
  const handleSelectWinner = async () => {
    ...
    try {
      await axios.patch(`http://147.93.27.172:3001/api/bids/${bidId}/select-winner`, {
            ...
            <div className="mt-2">
              ...
              <div className="flex gap-2">
                {bid.images.map((image, index) => (
                  <img
                    key={index}
                    src={`http://147.93.27.172:3001${image.path}`}
