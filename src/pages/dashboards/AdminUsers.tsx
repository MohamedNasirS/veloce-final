...
const AdminUsers = () => {
  ...
  useEffect(() => {
    axios
      .get('http://0.0.0.0:3001/api/admin/users')
      .then((res) => {
  ...
  const handleStatusChange = async (userId: string, newStatus: string) => {
    try {
      await axios.patch(`http://0.0.0.0:3001/api/admin/users/${userId}/status`, {
        status: newStatus,
      });
      setUsers((prev) =>
        prev.map((user) =>
  ...
  const handleViewDocuments = async (userId: string) => {
    try {
      const res = await axios.get(`http://0.0.0.0:3001/api/admin/users/${userId}/documents`);
