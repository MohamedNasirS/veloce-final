...
const GatePass: React.FC = () => {
  ...
  const fetchBidDetails = async () => {
    try {
      const response = await fetch(`http://147.93.27.172:3001/api/bids/${bidId}`);
  ...
  const fetchGatePassStatus = async () => {
    try {
      const response = await fetch(`http://147.93.27.172:3001/api/bids/${bidId}/gate-pass?userId=${user?.id}`);
      if (response.ok) {
  ...
  const handleGatePassUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await fetch(`http://147.93.27.172:3001/api/bids/${bidId}/gate-pass`, {
        method: 'POST',
        body: formData,
      });
  ...
  const handleViewGatePass = () => {
    if (gatePassPath) {
      window.open(`http://147.93.27.172:3001${gatePassPath}`, '_blank');
    }
  };

  const handleDownloadGatePass = () => {
    if (gatePassPath && bid) {
      const fullUrl = `http://147.93.27.172:3001${gatePassPath}`;
