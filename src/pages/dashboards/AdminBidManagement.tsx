...
const AdminBidManagement = () => {
  ...
  useEffect(() => {
    axios.get('http://0.0.0.0:3001/api/bids')
  ...
  const handleApprove = async (bidId: string) => {
    try {
      await axios.patch(`http://0.0.0.0:3001/api/bids/${bidId}/approve`);
  ...
  const handleCancel = async (bidId: string) => {
    try {
      await axios.patch(`http://0.0.0.0:3001/api/bids/${bidId}/cancel`);
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      {bid.images.map((img: any) => (
                        <img
                          key={img.id}
                          src={`http://0.0.0.0:3001${img.path}`}
