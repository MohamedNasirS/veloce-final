...
const GatePassList: React.FC = () => {
  ...
  const fetchClosedBidsWithWinners = async () => {
    try {
      ...
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/bids/creator/${user?.id}`, {
        headers: {
        ...
        closedBidsWithWinners.map(async (bid: BidWithGatePass) => {
          try {
            ...
            const gatePassResponse = await axios.get(
              `${import.meta.env.VITE_API_URL}/api/bids/${bid.id}/gate-pass?userId=${user?.id}`,
              {
                headers: {
                  'Content-Type': 'application/json',
                  'Authorization': user?.token ? `Bearer ${user?.token}` : undefined,
