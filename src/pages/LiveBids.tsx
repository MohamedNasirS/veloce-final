...
const socket = io('http://147.93.27.172');
...
const LiveBids = () => {
  ...
  const fetchLiveBids = async () => {
    setLoading(true);
    try {
      const res = await axios.get('http://147.93.27.172/api/bids/approved');
...
const handleBidUpdated = (updatedBid: any) => {
  ...
  const handleBidSubmit = async () => {
    ...
    try {
      const updatedBid = await axios.get(`http://147.93.27.172/api/bids/${selectedBid.id}`);
      ...
      const response = await axios.patch(`http://147.93.27.172/api/bids/${selectedBid.id}/bid`, {
        userId: user.id,
        amount,
      });
